...
```bash
# 默认 table 格式
pingcode-client project workitem list --project-id {pid}

# JSON 格式
pingcode-client project workitem list --project-id {pid} --output json

# 原始 API 响应
pingcode-client project workitem list --project-id {pid} --raw --output json
```
...
| **工作项类型 (Type)** | `pingcode-client project metadata types --project-id {pid}` | 如 `task`, `bug`, `story` |
| **状态 (State)** | `pingcode-client project metadata statuses` | 状态流转需符合工作流 |
| **优先级 (Priority)** | `pingcode-client project metadata priorities --project-id {pid}` | 需指定项目 ID |
| **负责人 (Assignee)** | `pingcode-client project metadata members --project-id {pid}` | 获取项目成员 ID |
| **迭代 (Sprint)** | `pingcode-client project iteration list --project-id {pid}` | 获取迭代 ID |
| **看板 (Board)** | `pingcode-client project kanban list --project-id {pid}` | 获取看板 ID |
| **看板栏 (Entry)** | `pingcode-client project kanban entries --project-id {pid} --board-id {bid}` | 获取具体的列 ID |
| **标签 (Tag)** | `pingcode-client project metadata tags list` | 获取标签 ID |
| **父工作项 (Parent)** | `pingcode-client project workitem list --project-id {pid}` | 获取已有工作项 ID |
...
1. **创建新标签**:
    ```bash
    pingcode-client project metadata tags create --name "重要"
    ```
...
```bash
pingcode-client project workitem list \
  --project-id 69beeb90a36e928af7cd9dab \
